    ...
    ** Function should be changed to use ATOMIC UNITS instead of ANGSTROMS **
    ...
    with open(filename, "wt") as f:
        ...
        if calctype=='excit':
            ...
            f.write("".join([" Symmetry=(Loose,Follow) scf=(",tight_str,"XQC) GFPrint GFInput"]))
...
def outputComplex2TAPBS(Complex,grid,layer_point,layer_nvec,outputfolder,eps_cav=1.0,eps_solv=1.002,eps_layer=2.0,solv_rad=1.4,NZCH=[0,-1],verbose=True,state='transition',replicate=False,Nrep=3,RepStep=1.1):
    ''' Function which reads list of molecules and output all nessesary files
    ...
    ** RepStep and kwargs using ATOMIC UNITS and grid and layer definition using ANGSTROMS.
    ...
    from QChemTool.QuantumChem.interaction import charge_charge
    
    outname="".join([outputfolder,'TrEsp_coupl.dat'])
    if state is None:
        charge1 = Complex[0].esp_trans
        charge2 = Complex[1].esp_trans
    else:
        charge1 = Complex[0].get_charge(state=state)
        charge2 = Complex[1].get_charge(state=state)
    struc1 = Complex[0].coor._value
    struc2 = Complex[1].coor._value
    E_trEsp = charge_charge(struc1,charge1,struc2,charge2,'cm-1')
    if verbose:
        print(E_trEsp,'cm-1')
    ...
    for ii in range(len(MolNameUnq)):
        ...
        for jj in range(len(Complex)):
            if Complex[jj].name==MolNameUnq[ii]:
                struc = Complex[jj]
                #if isinstance(struc,Defect):
                if state is None:
                    charge = struc.esp_trans
                else:
                    charge = struc.get_charge(state=state)
                with open(outname, "wt") as f:
                    # Total charge output
                    f.write("{:4.2f} pK R \n".format(pK1))
                    # Molname, atomname_unq, charge, 0.000000
        ...
        for ii in range(len(Complex)):
            ...
            with position_units('Angstrom'):
                ...
                for jj in range(struc.nat):
                    ...
                    f.write(' A')
                    f.write("{:>4d}".format(ii+1))
                    f.write("{:12.3f}{:8.3f}{:8.3f}".format(coor[jj,0],coor[jj,1],coor[jj,2]))            
                ...
                with position_units('Angstrom'):
                    ...
                    for kk in range(Nrep):
                        ...
                        for jj in range(struc.nat):
                            ...
                            f.write("{:12.3f}{:8.3f}{:8.3f}".format((coor[jj,0]+vec[0]),(coor[jj,1]+vec[1]),(coor[jj,2]+vec[2])))            
                            f.write("{:>6.3f}{:>6.3f}".format(0.000,struc.vdw_rad[jj]))
                            f.write('      COMP \n')
                        counter_mol+=1
                    for kk in range(Nrep):
...
def outputComplex2TAPBS_old(Complex,grid,layer_point,layer_nvec,outputfolder,eps_cav=1.0,eps_solv=1.002,eps_layer=2.0,solv_rad=1.4,NZCH=[0,-1],verbose=True,typ='Transition'):
    ''' Function which reads list of molecules and output all nessesary files
    for interaction energy calculation with TAPBS. TAPBS calculates interaction 
    energy between individual chromophores in continuous polarizable dielectric
    medium with membrane. It is alsso possible to define 2D slab dielectric medium.
