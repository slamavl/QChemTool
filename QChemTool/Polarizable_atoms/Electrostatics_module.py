...
#import Polarizable_atoms.Polarization_module as PolMod
...
class Electrostatics:    
    ...
    def get_EnergyShift(self):
        ...
        Eshift,dist=inter.charge_charge(Def_coor,Def_charge,Env_coor,Env_charge,'Hartree')
...
def PrepareMolecule_1Def(filenames,indx,FG_charges_in,ChargeType='qchem',verbose=False):
    ...
    if ChargeType=='qchem' or ChargeType=='qchem_all':
        coor_grnd,charge_grnd,at_type=read.read_TrEsp_charges(filenameESP_grnd)
        coor_exct,charge_exct,at_type=read.read_TrEsp_charges(filenameESP_exct)
    if ChargeType=='AMBER':
        coor_grnd,Bond,charge_grnd,AtName,AtType,Molecule,Molname,info=read.read_mol2(filenameESP_grnd)
        coor_exct,Bond,charge_exct,AtName,AtType,Molecule,Molname,info=read.read_mol2(filenameESP_exct)
    if ChargeType=='gaussian':
        Points,ESP,coor_grnd,charge_grnd=read.read_gaussian_esp(filenameESP_grnd,output_charge=True)
        Points,ESP,coor_exct,charge_exct=read.read_gaussian_esp(filenameESP_exct,output_charge=True)
#    mol.center_molecule(indx_center1,indx_x1,indx_y1)
    ...
    mol.guess_bonds()
    NBonds=np.zeros(len(mol.mol_spec['Bonds']),dtype='i')
    for ii in range(len(mol.mol_spec['Bonds'])):
        if mol.at_spec['AtType'][mol.mol_spec['Bonds'][ii,0]]=='C' and mol.at_spec['AtType'][mol.mol_spec['Bonds'][ii,1]]=='F':
            NBonds[mol.mol_spec['Bonds'][ii,0]]+=1
    
    # Assing type for every atom - in this step all fluorines will have the same type
    Elstat_Type=[]
    for ii in range(mol.at_spec['NAtoms']):
        if mol.at_spec['AtType'][ii]=='C':
            if NBonds[ii]==2:
    ...
    for ii in range(len(mol.mol_spec['Bonds'])):
        if Elstat_Type[mol.mol_spec['Bonds'][ii,0]]=='CF2' and Elstat_Type[mol.mol_spec['Bonds'][ii,1]]=='FC':
            Elstat_Type[mol.mol_spec['Bonds'][ii,1]]='F2C'
    ...
    Elstat_Charge=np.zeros(mol.at_spec['NAtoms'],dtype='f8')
    for ii in range(mol.at_spec['NAtoms']):
    ...
    Elstat_mol=Electrostatics(mol.at_spec['Coor'],Elstat_Charge,Elstat_Type) 
    
    return Elstat_mol,mol.at_spec['AtType']
