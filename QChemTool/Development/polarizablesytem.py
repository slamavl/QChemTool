...
from .polarization import Dielectric
...
class PolarizableSystem(UnitsManaged):    
    ...
    def _build_polar(self,struc,params):
        try:
            VinterFG = params["VinterFG"]
        except:
            VinterFG = 0.0
        
        coarse_grain = params["coarse_grain"]
        
        PolCoor,Polcharge,PolType = _prepare_polar_structure(struc,coarse_grain,verbose=False)
        polar={}
        polar['AlphaE']=np.zeros((len(PolCoor),3,3),dtype='f8')
        polar['Alpha_E']=np.zeros((len(PolCoor),3,3),dtype='f8')
        polar['BetaEE']=np.zeros((len(PolCoor),3,3),dtype='f8')
        polar['Alpha_st']=np.zeros((len(PolCoor),3,3),dtype='f8')
        dipole_init = np.zeros((len(PolCoor),3),dtype='f8')
        
        mol_polar=Dielectric(PolCoor,PolType,Polcharge,dipole_init,polar['AlphaE'],
                         polar['Alpha_E'],polar['Alpha_st'],polar['BetaEE'],
                         VinterFG,CoarseGrain=coarse_grain)
