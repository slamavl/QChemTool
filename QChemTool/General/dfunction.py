...
#FIXME Add numerical integration possibility
...
class DFunction():
    ...
    get_Fourier_transform()
        ...
        Fourier transformation defined as:
        F(om) = int{t,-infty,infty}{f(t)*exp[i*om*t]} and
        f(t) =  int{om,-infty,infty}{F(om)*exp[i*om*t]}
    get_inverse_Fourier_transform()
        Returns inverse Fourier transformed DFunction
        Fourier transformation defined as:
        F(om) = 1/(2*pi) * int{t,-infty,infty}{f(t)*exp[-i*om*t]} and
        f(t) = 1/(2*pi) * int{om,-infty,infty}{F(om)*exp[-i*om*t]}
    plot()
        Plots the function
...
#                y[t.length-1] = 0.0
    ...
    def get_rFourier_transform(self):
        """Returns Fourier transform of the DFunction
        """
        ...
        if isinstance(t, TimeAxis):
            ...
            w = t.get_rFrequencyAxis()

            if t.atype == "upper-half":
                Y = w.length*numpy.fft.fftshift(numpy.fft.irfft(y))*t.step
    ...
    def get_inverse_rFourier_transform(self):
        """Returns inverse Fourier transform of the DFunction
        """
        ...
        # FIXME add option for time axis
        
        if isinstance(t, FrequencyAxis):

            w = t
            t = w.get_rTimeAxis()
            
            #np.fft.rfft(np.fft.fftshift(np.real(Y)))*(omega[1]-omega[0])/(np.pi*2.0)
            Y = numpy.fft.rfft(numpy.fft.fftshift(numpy.real(y)))*w.step/(numpy.pi*2.0)
